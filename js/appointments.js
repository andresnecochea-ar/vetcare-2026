...
function openDayDetail(dateStr) {
  ...
  showModal(`
    ...
    <div class="modal-body">
      ...
      ${appts.map(a => { const p = db.pets.find(x=>x.id===a.petId); return `<div class="reminder-item"><div class="info"><strong>${a.time||''} · ${p?`<button type="button" class="link-inline" onclick="closeModal();openPetDetail('${p.id}')">${escapeHtml(p.name)}</button>`:''}</strong><small>${escapeHtml(a.type||'')} ${a.vet?'· '+escapeHtml(a.vet):''}</small></div></div>`; }).join('')}
      ${groom.length ? '<h3 style="margin:12px 0 8px">Peluquería</h3>' : ''}
      ${groom.map(a => { const p = db.pets.find(x=>x.id===a.petId); return `<div class="reminder-item"><div class="info"><strong>${a.time||''} · ${p?`<button type="button" class="link-inline" onclick="closeModal();openPetDetail('${p.id}')">${escapeHtml(p.name)}</button>`:''}</strong><small>${escapeHtml(a.service||'')} ${a.groomer?'· '+escapeHtml(a.groomer):''}</small></div></div>`; }).join('')}
      ${rems.length ? '<h3 style="margin:12px 0 8px">Avisos</h3>' : ''}
      ${rems.map(r => { const p = db.pets.find(x=>x.id===r.petId); return `<div class="reminder-item soon"><div class="info"><strong>${escapeHtml(r.title)}</strong><small>${p?`<button type="button" class="link-inline" onclick="closeModal();openPetDetail('${p.id}')">${escapeHtml(p.name)}</button> · `:''}${escapeHtml(r.notes||'')}</small></div><button class="btn btn-sm" onclick="completeReminder('${r.id}')">✓</button></div>`; }).join('')}
    </div>
    <div class="modal-footer">
